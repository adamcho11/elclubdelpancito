      ...
      <div className="min-h-screen flex items-center justify-center bg-oven-950">
        <div className="text-center max-w-md mx-auto px-4 py-20">
          ...
          <p className="text-cream-dark/70 mb-8">
            Recibimos tu comprobante de pago. Lo revisaremos y te contactaremos pronto.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link href="/panel" className="px-8 py-3 bg-gradient-ember text-white font-semibold rounded-xl text-sm hover:shadow-xl hover:shadow-ember/30 transition-all duration-300">Ver mi panel</Link>
            <Link href="/" className="px-8 py-3 border border-oven-500/30 text-cream-dark/90 font-semibold rounded-xl text-sm hover:border-cream-dark/40 hover:text-cream-light transition-all duration-300">Volver al inicio</Link>
          </div>
        </div>
      </div>
    )
    ...
    <div className="min-h-screen">
      <section className="py-20 bg-gradient-oven border-b border-oven-700/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl font-bold text-cream-light mt-3 mb-4">Finalizar suscripción</h1>
          <p className="text-cream-dark/70 max-w-xl mx-auto">Elegí tu plan, agregá extras, adjuntá el comprobante y listo.</p>
        </div>
      </section>
          ...
          <form onSubmit={handleSubmit} className="space-y-10">
            ...
            <div>
              ...
              <div className="grid gap-3">
                {plans.map((plan) => (
                  <label key={plan.id} className={`flex items-center gap-4 p-4 rounded-xl border cursor-pointer transition-all duration-200 ${selectedPlan === plan.id ? "border-ember bg-ember/5" : "border-oven-600/30 bg-gradient-card hover:border-oven-500/40"}`}>
              ...
              {preselectedPlan && (
                <p className="text-cream-dark/40 text-xs mt-2">
                  Plan pre-seleccionado: {preselectedPlan.name}. Podés cambiarlo.
                </p>
              )}
            </div>
            ...
            {selectedPlan && (
              <div>
                <h2 className="text-lg font-semibold text-cream-light mb-2">2. Agregar productos extra</h2>
                <p className="text-cream-dark/50 text-xs mb-4">Opcional. Cada producto extra se suma al total del plan.</p>
                <div className="grid gap-2">
                  {complements.map((comp) => {
                    const isSelected = extras.has(comp.id)
                    return (
                      <button
                        key={comp.id}
                        type="button"
                        onClick={() => toggleExtra(comp.id)}
                        className={`flex items-center gap-3 p-3 rounded-xl border text-left transition-all duration-200 ${isSelected ? "border-ember bg-ember/5" : "border-oven-600/20 bg-gradient-card hover:border-oven-500/40"}`}
                      >
                        <div className={`w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 transition-colors ${isSelected ? "border-ember bg-ember" : "border-oven-500/40"}`}>
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="text-cream-light text-sm">{comp.name}</div>
                          <div className="text-cream-dark/50 text-xs">{comp.brand}</div>
                        </div>
                        <span className="text-cream-dark/60 text-sm font-medium shrink-0">+Bs. {comp.price}</span>
                      </button>
                    )
                  })}
                </div>
                {extras.size > 0 && (
                  <div className="mt-3 p-3 rounded-xl bg-ember/5 border border-ember/15 text-center">
                    <p className="text-cream-dark/70 text-sm">
                      {extras.size} producto{extras.size > 1 ? "s" : ""} extra{extras.size > 1 ? "s" : ""}: +Bs. {extrasTotal}
                    </p>
                  </div>
                )}
              </div>
            ...
            {selectedPlan && (
              <div className="p-4 rounded-xl bg-gradient-card border border-oven-600/20 text-center">
                <p className="text-cream-dark/60 text-xs mb-1">Total a pagar</p>
                <p className="text-cream-light text-2xl font-bold">
                  Bs. {total}
                  <span className="text-cream-dark/50 text-sm font-normal">/semana</span>
                </p>
                {extrasTotal > 0 && (
                  <p className="text-cream-dark/50 text-xs mt-1">
                    Plan: Bs. {planPrice} + Extras: Bs. {extrasTotal}
                  </p>
                )}
              </div>
            )}
            ...
            <div>
              ...
              <div className="p-6 rounded-xl bg-gradient-card border border-oven-600/20 text-center">
                <p className="text-cream-dark/70 text-sm mb-4">Escaneá el código QR con tu app bancaria.</p>
                ...
                <p className="text-cream-dark/50 text-xs">Realizá el pago por Bs. {total} y guardá el comprobante.</p>
              </div>
            </div>

            {/* Step 4: Notes */}
            <div>
              <h2 className="text-lg font-semibold text-cream-light mb-4">4. Notas adicionales</h2>
              <textarea value={notas} onChange={(e) => setNotas(e.target.value)} rows={3} className="w-full px-4 py-3 bg-oven-900 border border-oven-600/40 rounded-xl text-cream-light placeholder:text-cream-muted/50 focus:outline-none focus:border-ember/60 transition-colors text-sm resize-none" placeholder="Horario preferido, alergias, instrucciones especiales..." />
            </div>

            {/* Step 5: Upload receipt */}
            <div>
              <h2 className="text-lg font-semibold text-cream-light mb-4">5. Adjuntá tu comprobante</h2>
              <div className="p-6 rounded-xl bg-gradient-card border border-oven-600/20">
                <label htmlFor="recibo" className={`flex flex-col items-center gap-3 p-6 rounded-xl border-2 border-dashed cursor-pointer transition-colors ${recibo ? "border-emerald-600/30 bg-emerald-600/5" : "border-oven-600/30 hover:border-oven-500/40 hover:bg-oven-800/20"}`}>
                  {recibo ? (
                    <>
            ...
            <button type="submit" disabled={loading || !selectedPlan} className="w-full py-4 bg-gradient-ember text-white font-semibold rounded-xl text-base hover:shadow-xl hover:shadow-ember/30 transition-all duration-300 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed">
              {loading ? "Enviando..." : !selectedPlan ? "Seleccioná un plan" : `Enviar comprobante (Bs. ${total})`}
            </button>
          </form>
        </div>
      </section>
