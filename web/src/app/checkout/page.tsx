...
const STEPS = ["Plan", "Extras", "Revisar", "Pagar"]
      ...
      <section className="py-16 sm:py-20 bg-gradient-oven border-b border-oven-700/20">
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <div className="flex items-center justify-center gap-1 sm:gap-2 max-w-md mx-auto">
            {STEPS.map((label, i) => (
              <div key={label} className="flex items-center gap-1 sm:gap-2 flex-1 last:flex-[0]">
                <button
                  type="button"
                  onClick={() => { if (i <= step || (i === step + 1 && canNext())) setStep(i) }}
                  className={`flex flex-col items-center gap-1 min-w-0 ${i > step ? "pointer-events-none" : ""}`}
                >
                  <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all ${i < step ? "bg-emerald-600 text-white" : i === step ? "bg-ember text-white scale-110" : "bg-oven-700 text-cream-muted/40"}`}>
                    {i < step ? "✓" : i + 1}
                  </div>
                  <span className={`text-[10px] sm:text-xs whitespace-nowrap ${i === step ? "text-cream-light font-medium" : i < step ? "text-emerald-400/70" : "text-cream-muted/30"}`}>{label}</span>
                </button>
                {i < STEPS.length - 1 && (
                  <div className={`flex-1 h-0.5 rounded-full transition-all ${i < step ? "bg-emerald-600" : "bg-oven-700"}`} />
                )}
              </div>
            ))}
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            {step === 0 && (
              <div className="space-y-4">
                ...
                {preselectedPlan && <p className="text-cream-dark/40 text-xs">Plan pre-seleccionado: {preselectedPlan.name}</p>}
              </div>
            )}
            ...
            {step === 1 && (
              <div className="space-y-3">
                ...
                {complements.map((comp) => {
                  ...
                  return (
                    <button key={comp.id} type="button" onClick={() => toggleExtra(comp.id)} className={`w-full flex items-center gap-3 p-2.5 sm:p-3 rounded-xl border text-left transition-all ${isSel ? "border-ember bg-ember/5" : "border-oven-600/20 bg-gradient-card hover:border-oven-500/40"}`}>
                        ...
                        {Array.from(extras).map((id) => {
                          ...
                          return c ? (
                            <div key={id} className="flex justify-between text-sm">
                              <span className="text-cream-dark/70">+ {c.name}</span>
                              <span className="text-cream-dark/60">Bs. {c.price}</span>
                            </div>
                          ) : null
                        })}
                      </div>
            ...
            {error && (
              <div className="mt-4 p-3 rounded-xl bg-red-900/20 border border-red-700/30 text-red-400 text-sm text-center">{error}</div>
            )}

            {/* Navigation buttons */}
            <div className="mt-8 flex gap-3">
              {step > 0 && (
                <button type="button" onClick={() => setStep(step - 1)} className="px-6 py-3 border border-oven-500/30 text-cream-dark/90 font-semibold rounded-xl text-sm hover:border-cream-dark/40 hover:text-cream-light transition-all">
                  Atrás
                </button>
              )}
              {step < 3 && (
                <button type="button" onClick={() => { if (canNext()) setStep(step + 1) }} disabled={!canNext()} className="ml-auto px-8 py-3 bg-gradient-ember text-white font-semibold rounded-xl text-sm hover:shadow-xl hover:shadow-ember/30 transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed">
                  {step === 0 && !selectedPlan ? "Elegí un plan" : step === 0 ? "Siguiente: Extras" : `Siguiente: ${STEPS[step + 1]}`}
                </button>
              )}
              {step === 3 && (
